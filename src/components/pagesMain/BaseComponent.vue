<template>
    <div class="text-3xl w-full h-full bg-white rounded-t-lg flex justify-between items-center px-4  flex-col pt-16">
        <div class="flex justify-start items-center flex-col gap-2 w-full overflow-scroll mb-4">
            ...
            <ItemBaseComponent name="Процент дефектных швов" :data="data[0]" :value="value" typeChart="line" :adj="adj"
                :int="int" :geo="geo" :pro="pro" :non="non" />
...
const value = ref(0)
onMounted(() => {
    value.value = props.adj + props.def + props.per + props.det + props.detDet
})
</script>