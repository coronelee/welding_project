...
<input type="file" id="camera" accept="image/*" capture="camera" class="hidden" @change="onFileChange">
     <div class="w-full h-full flex justify-center items-center flex-col p-2 gap-2 animate-[showMain_0.5s_ease_1]">
        <div class="text-3xl font-mono w-[calc(100%-58px)] h-full bg-gray-300 bg-center bg-no-repeat bg-contain" :style="{backgroundImage: `url(${imageSrc})`}"></div>
        <div class="w-full h-[50px] flex gap-2 [&>button]:w-full [&>button]:h-full   [&>button]:bg-contain [&>button]:bg-center [&>button]:bg-no-repeat rounded-lg">
            <button class="bg-[url('/images/download.svg')]"  @click="downloadImage('file')"></button>
